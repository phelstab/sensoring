...
class FakeDataGenerator:
    def __init__(self):
        ...
        self.bvp_data = lambda: random.uniform(0, 100)
        self.gsr_data = lambda: random.uniform(0, 20)
        self.tmp_data = lambda: random.uniform(30, 40)
        self.ibi_data = lambda: random.uniform(0.5, 1.5)
        self.hr_data = lambda: random.uniform(60, 100)
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle('Fake Empatica Data Generator')
    ...
    def open_csv_files(self):
        self.files = {##
            'acc': open(os.path.join(self.recording_folder, 'empatica_acc_data.csv'), 'w', newline=''),
            'bvp': open(os.path.join(self.recording_folder, 'empatica_bvp_data.csv'), 'w', newline=''),
            'gsr': open(os.path.join(self.recording_folder, 'empatica_gsr_data.csv'), 'w', newline=''),
            'tmp': open(os.path.join(self.recording_folder, 'empatica_temp_data.csv'), 'w', newline=''),
            'ibi': open(os.path.join(self.recording_folder, 'empatica_ibi_data.csv'), 'w', newline=''),
            'hr': open(os.path.join(self.recording_folder, 'empatica_hr_data.csv'), 'w', newline='')
        }
    ...
    def generate_and_write_data(self):
        ...
        for data_type in ['bvp', 'gsr', 'tmp', 'ibi', 'hr']:
